import { Button, Flex, Icon, IconButton, Switch, Text, VStack } from "@chakra-ui/react";
import {
  Drawer,
  DrawerBody,
  DrawerFooter,
...
const changeTheme = () => {
  ...
  return (
    ...
    <Drawer isOpen={isOpened} onClose={toggleDrawer} placement="left">
      ...
      {navs.map((i) => {
        return (
              <Flex key={i.id}  p={2} justifyContent="start" w="100%" _hover={{
                bgColor: "teal",
                color: "white" 
              }}>
              <Icon as={i.icon} mr={2} boxSize={6}></Icon>
              <Text textAlign="start" ml={4} fontSize="md" fontWeight="800" cursor="pointer" >{i.name}</Text>
          </Flex>
      ...
      <DrawerFooter flexFlow="column">
        ...
        <Switch isChecked={isDark} onChange={changeTheme} mb={4}>Dark Theme</Switch>
        <Button w="100%" variant="solid" colorScheme="primary">Logout</Button>
      </DrawerFooter>
      </DrawerContent>
    </Drawer>
